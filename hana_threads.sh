...
## thread_count must be a positive integer; cap it so a typo can't flood the output
if ! [[ "${thread_count}" =~ ^[1-9][0-9]*$ ]]; then
        die "thread_count must be a positive integer, got '${thread_count}'"
fi
if (( thread_count > 100 )); then
        die "thread_count must be 100 or less, got '${thread_count}'"
fi
...
## Collect non-empty hashes into an array (an empty result gives zero entries)
hash_list=()
