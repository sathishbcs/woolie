...
if [ "$#" -eq 8 ] || [ "$#" -eq 9 ]; then
        ...
        db_tenant=${3}       ## tenant database name for hdbsql -d (e.g. SEC on an S08
                             ## installation). Pass the SID itself, or 'same', when the
                             ## tenant name matches the installation SID. SYSTEMDB queries
                             ## the system database instead of a tenant.
        dbUsername=${4}      ## schema / connect user name
        db_password=${5}     ## DB password, or 'none' to use hdbuserstore key
        begin_time=${6}      ## BEGIN_TIME for CPU/Memory query window
        end_time=${7}        ## END_TIME for CPU/Memory query window
        script_dir=${8}/hana_dbop_comparison
        peak_window_min=${9:-120}   ## OPTIONAL: half-width of the peak window in MINUTES
                                    ##           (default 120 = +/- 2 hours around the peak)

        ## Convenience: an empty value or the literal 'same'/'none' means the tenant
        ## name equals the installation SID, which keeps older landscapes simple.
        ...
        echo "Usage: $0 db_sid db_inst_no tenant_db_sid dbUsername db_password begin_time end_time script_dir [peak_window_minutes]"
        ...
        echo "       peak_window_minutes is optional and defaults to 120 (+/- 2 hours)"
        exit 1
fi

## Validate the window value up front so a typo fails clearly rather than
## silently producing a nonsensical BEGIN_TIME/END_TIME later on.
if ! [[ "${peak_window_min}" =~ ^[0-9]+$ ]] || [[ "${peak_window_min}" -eq 0 ]]; then
        die "peak_window_minutes must be a positive whole number of minutes, got '${peak_window_min}'"
fi
peak_window_sec=$(( peak_window_min * 60 ))

## Validate the tenant name the same way, so a bad value fails here instead of
## producing a confusing hdbsql connection error.
...
# --- Embedded SQL (HANA_Resources_CPUAndMemory_2.00.060+, modified: ORDER BY
#     CPU_PCT DESC first) with BEGIN_TIME / END_TIME as __placeholders__ ---
...
- M_HOST_RESOURCE_UTILIZTION.INSTANCE_TOTAL_MEMORY_USED_SIZE may show wrong temporary spikes when PMEM / FRO is used (bug 344877)
...
- 2026/07/29:  2.1 (custom: final ORDER BY changed to CPU_PCT DESC first, to surface the peak-CPU
                     snapshot at the top of the result set for the specified BEGIN_TIME/END_TIME window)
[INVOLVED TABLES]
...
- The final ORDER BY (bottom of the statement) has been changed to sort by CPU_PCT DESC first,
  so the row with the highest combined user+system CPU utilization in the BEGIN_TIME/END_TIME
  window appears at the very top of the result set - making it easy to identify the exact
  snapshot time where CPU peaked.
[EXAMPLE OUTPUT]
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
# --- Build the final SQL by substituting BEGIN_TIME / END_TIME placeholders ---
# The tenant name is part of the generated file names so that parallel runs
# against different tenants of the same instance cannot overwrite each other.
ts="$(date +%Y%m%d_%H%M%S)"
TMP_SQL="${script_dir}/hana_cpu_report_${db_tenant}_${ts}.sql"
OUTPUT_FILE="${script_dir}/hana_cpu_report_${db_tenant}_${ts}.out"

# Escape sed-special characters (&, /, and \) in the supplied values, just in case.
...
# --- Execute via the instance's local hdbsql binary ---
...
if [[ "${db_password}" == "none" || "${db_password}" == "NONE" ]]; then
  ...
  # script (e.g. HDBUSERSTORE_KEY=SSSTENANT).
  HDBUSERSTORE_KEY="${HDBUSERSTORE_KEY:-${dbUsername}}"
  HDBSQL_ARGS+=(-U "${HDBUSERSTORE_KEY}")
else
  HDBSQL_ARGS+=(-u "${dbUsername}" -p "${db_password}")
fi
...
# --- Extract the peak CPU_PCT snapshot time and derive the peak window ---
# The query's ORDER BY is CPU_PCT DESC first, so the first SNAPSHOT_TIME
# value found in the output belongs to the highest-CPU row.
# The window half-width comes from peak_window_min (default 120 minutes), and
# the printed labels below are generated from that same value, so the text can
# never disagree with the arithmetic.
peak_snapshot="$(grep -oE '[0-9]{4}/[0-9]{2}/[0-9]{2}[[:space:]][0-9]{2}:[0-9]{2}:[0-9]{2}' "${OUTPUT_FILE}" | head -1)"

if [[ -z "${peak_snapshot}" ]]; then
  echo "WARNING: No SNAPSHOT_TIME found in ${OUTPUT_FILE} (no rows returned?). Skipping peak-window calculation." >&2
else
  peak_epoch="$(date -d "${peak_snapshot}" +%s)" || die "Could not parse peak snapshot time '${peak_snapshot}'"
  new_begin_time="$(date -d "@$((peak_epoch - peak_window_sec))" '+%Y/%m/%d %H:%M:%S')"
  new_end_time="$(date -d "@$((peak_epoch + peak_window_sec))" '+%Y/%m/%d %H:%M:%S')"

  # Human-readable description of the window, e.g. "2h", "30m", "1h30m"
  if (( peak_window_min % 60 == 0 )); then
    window_label="$(( peak_window_min / 60 ))h"
  elif (( peak_window_min > 60 )); then
    window_label="$(( peak_window_min / 60 ))h$(( peak_window_min % 60 ))m"
  else
    window_label="${peak_window_min}m"
  fi

  echo
  echo "Tenant database               : ${db_name}"
  echo "Peak CPU_PCT SNAPSHOT_TIME    : ${peak_snapshot}"
  ...
  echo "##gbStart##peakcpustartTime##splitKeyValue##${new_begin_time// /_}##splitKeyValue##string##gbEnd##"
  echo "##gbStart##peakcpuendTime##splitKeyValue##${new_end_time// /_}##splitKeyValue##string##gbEnd##"
fi
