...
if [ "$#" -eq 7 ] || [ "$#" -eq 8 ]; then
        ...
        dbUsername=${3}      ## schema / connect user name
        db_password=${4}     ## DB password, or 'none' to use hdbuserstore key <dbUsername>
        begin_time=${5}      ## BEGIN_TIME for CPU/Memory query window
        end_time=${6}        ## END_TIME for CPU/Memory query window
        script_dir=${7}/hana_dbop_comparison
        peak_window_min=${8:-120}   ## OPTIONAL: half-width of the peak window in MINUTES
                                    ##           (default 120 = +/- 2 hours around the peak)
else
        echo "Parameter missing"
        echo "Usage: $0 db_sid db_inst_no dbUsername db_password begin_time end_time script_dir [peak_window_minutes]"
...
TMP_SQL="${script_dir}/hana_cpu_report_${ts}.sql"
OUTPUT_FILE="${script_dir}/hana_cpu_report_${ts}.out"
...
echo "client=${clientno} sid=${db_sid} inst=${db_inst_no} db=${db_name}" >&2
...
if [[ "${db_password}" == "none" || "${db_password}" == "NONE" ]]; then
  # hdbuserstore key is assumed to be named after the schema/connect user.
  # Adjust HDBUSERSTORE_KEY below if your stored key uses a different name.
  HDBUSERSTORE_KEY="${dbUsername}"
